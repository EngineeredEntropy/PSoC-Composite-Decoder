...
void PIXEL_CLK_Start(void) 
{
    ...
    PIXEL_CLK_CLKEN |= PIXEL_CLK_CLKEN_MASK;
	PIXEL_CLK_CLKSTBY |= PIXEL_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: PIXEL_CLK_Stop
********************************************************************************
*
* Summary:
...
void PIXEL_CLK_Stop(void) 
{
    ...
    PIXEL_CLK_CLKEN &= (uint8)(~PIXEL_CLK_CLKEN_MASK);
	PIXEL_CLK_CLKSTBY &= (uint8)(~PIXEL_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: PIXEL_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((PIXEL_CLK_CLKEN & PIXEL_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = PIXEL_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = PIXEL_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* PIXEL_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(PIXEL_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(PIXEL_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: PIXEL_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: PIXEL_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = PIXEL_CLK_GetSourceRegister();
    uint16 oldDivider = PIXEL_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = PIXEL_CLK_CLKEN & PIXEL_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((PIXEL_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: PIXEL_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(PIXEL_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: PIXEL_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    PIXEL_CLK_MOD_SRC |= modeBitMask & (uint8)PIXEL_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: PIXEL_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    PIXEL_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(PIXEL_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: PIXEL_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return PIXEL_CLK_MOD_SRC & (uint8)(PIXEL_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: PIXEL_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = PIXEL_CLK_GetDividerRegister();
    uint8 oldSrc = PIXEL_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        PIXEL_CLK_MOD_SRC |= CYCLK_SSS;
        PIXEL_CLK_MOD_SRC =
            (PIXEL_CLK_MOD_SRC & (uint8)(~PIXEL_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: PIXEL_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return PIXEL_CLK_MOD_SRC & PIXEL_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: PIXEL_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    PIXEL_CLK_PHASE = clkPhase & PIXEL_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: PIXEL_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return PIXEL_CLK_PHASE & PIXEL_CLK_PHASE_MASK;
}
